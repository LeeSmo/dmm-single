...
public class ControllerAspect {
     ...
     */
    //@Pointcut("execution(* com.app.dmm.modules.*.*(..))")
    @Pointcut("execution(* com.app.dmm.modules.*.controller.*Controller.*(..))")
    ...
    public Object doAround(ProceedingJoinPoint call) throws Throwable {
        ...
        Object result = null;
        try {
            log.info("请求日志：{} >> [接口请求开始] 方法名：{} , 请求参数：{}", reqId, methodName, params);
            result = call.proceed();
            return result;

        } finally {
            long runTimes = System.currentTimeMillis() - start;
            log.info("请求日志：{} >> [接口请求结束] 方法名：{} , 请求耗时：{} ms", reqId, methodName, runTimes);
        }
    }
